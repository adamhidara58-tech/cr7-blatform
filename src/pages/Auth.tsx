...
}: {icon: React.ElementType;type?: string;placeholder: string;value: string;onChange: (e: React.ChangeEvent<HTMLInputElement>) => void;required?: boolean;minLength?: number;maxLength?: number;endIcon?: React.ReactNode;}) =>
    ...
    try {
      if (isSignUp) {
        ...
        if (error) {
          ...
          if (error.message?.includes('already registered')) msg = 'البريد الإلكتروني مسجل مسبقاً';else
          if (error.message?.includes('Invalid email')) msg = 'البريد الإلكتروني غير صالح';else
          if (error.message?.includes('Password')) msg = 'كلمة السر ضعيفة جداً';
        ...
        if (error) {
          ...
          if (error.message?.includes('Invalid login')) msg = 'البريد الإلكتروني أو كلمة السر غير صحيحة';else
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center relative overflow-hidden selection:bg-primary/30 px-4 py-8">
      ...
      <motion.div
        ...
        <div className="text-center">
          <h1 className="font-bold text-4xl sm:text-5xl text-gradient-gold tracking-tight">CR7 BLATFORM
